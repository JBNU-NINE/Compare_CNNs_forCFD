...
def get_regressive_predictions(model, dataset):
    ...
    with torch.no_grad():
        for i in range(n_samples):
            x = x.to(model.device)
            y_hat = model.model(x)
            predictions_arr.extend(y_hat.detach().cpu().numpy())
            test_y_arr.append(np.array(dataset[i][1]))
            x_list = x.squeeze(0).tolist()
            x_list.append(y_hat.squeeze(0))
            x_list.pop(0)
...
def plot_max_ae(predicted_y, test_y, channel_name):
    ...
    plt.savefig(os.path.join(save_path, f"{channel_name}_regressive_max_ae.png"))
...
log_version = "/home/ubuntu/ml-convection/src_v2/logs/lstm_autoencoder/absolute_paper"
